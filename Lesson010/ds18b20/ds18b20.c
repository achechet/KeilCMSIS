...
uint8_t ds18b20_Reset(void) 
		{
			uint16_t result;   
			// if((!(GPIOC->IDR & 1<<7))==0)  return 2;         //проверить линию на отсутствие замыкания
			SWI_LOW;
			Delay_us(480);
			SWI_FLOAT;
			Delay_us(550);		
			result =  (GPIOC->IDR & 1<<7);                 //прочитать шину 
			Delay_us(960);	
...
float ds18b20_Convert(uint16_t dt) 
		{
			float t;
			t = (float) ((dt&0x07FF) >> 4);		// Отбросим знаковые и дробные биты
